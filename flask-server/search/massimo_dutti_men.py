...
def gather_items():
    ...
    resp = requests.get("https://www.massimodutti.com/itxrest/2/catalog/store/34009453/30359501/category?languageId=100&appId=1", headers=headers)
    data = resp.json()
    categories = data["categories"]
    categoryId = 0
    for category in categories:
        if category["nameEn"] == "MEN":
            for subcategory in category["subcategories"]:
                if subcategory["nameEn"] == "NEW COLLECTION":
                    for subsubcategory in subcategory["subcategories"]:
                        if subsubcategory["nameEn"] == "SPECIAL PRICES" or subsubcategory["nameEn"] == "SALE":
                            for subsubsubcategory in subsubcategory["subcategories"]:
                                if subsubsubcategory["nameEn"] == "VIEW ALL":
                                    if subsubsubcategory["isRedirected"]:
                                        categoryId = subsubsubcategory["redirectCategoryId"]
                                    else:
