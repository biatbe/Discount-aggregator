...
def gather_items():
    ...
    resp = requests.get("https://api.hm.com/search-services/v1/nl_NL/listing/resultpage?page=1&pageId=/men/last-chance/view-all&page-size=72&categoryId=MEN_LASTCHANCE&touchPoint=DESKTOP&skipStockCheck=false", headers=headers)
    data = resp.json()
    ...
    for pageNum in range(1, total_pages + 1):
        resp = requests.get(f'https://api.hm.com/search-services/v1/nl_NL/listing/resultpage?page={pageNum}&pageId=/men/last-chance/view-all&page-size=72&categoryId=MEN_LASTCHANCE&touchPoint=DESKTOP&skipStockCheck=false', headers=headers)
        data = resp.json()
        for product in data["plpList"]["productList"]:
            stripped_product = {}
            stripped_product["id"] = product["id"]
            stripped_product["brand"] = "H&M"
            stripped_product["sectionName"] = "men"
